...
class LeadStatusHistoryWithSupposedEntry
{
	...
	{
		if (self::isAnalyticsPrepared())
		{
			$isStepperCompleted = Option::get("crm", 'statisticPreCalculateLeadSupposedHistoryFinish', 'N');
			if ($isStepperCompleted === 'Y')
			{
				self::copyHistoryFromExist($leadId);
			}
		}
	}

	public static function copyHistoryFromExist($leadId)
	{
	...
	private static function isAnalyticsPrepared()
	{
		global $DB;
		return Option::get('report', '~analytics_enabled', 'N') === 'Y' && $DB->TableExists('b_crm_lead_status_history_with_supposed');
	}
