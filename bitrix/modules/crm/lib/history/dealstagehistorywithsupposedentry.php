...
class DealStageHistoryWithSupposedEntry
{
	...
	public static function register($dealId)
	{
		...
		{
			$isStepperCompleted = Option::get("crm", 'statisticPreCalculateSupposedHistoryFinish', 'N');
			if ($isStepperCompleted === 'Y')
			{
				self::copyHistoryFromExist($dealId);
			}
		}
	}

	public static function copyHistoryFromExist($dealId)
	{
	...
	{
		global $DB;
		return Option::get('report', '~analytics_enabled', 'N') === 'Y' && $DB->TableExists('b_crm_deal_stage_history_with_supposed');
	}
