...
class EventHandler
{
	...
	public static function onAnalyticPageCollect()
	{
		...
		if (LeadSettings::isEnabled())
		{
			...
			$leadAnalytics->addFeedbackButton();
			$leadAnalytics->setDisabled(!VC\Helper\Analytic::isEnabledCrmAnalytics());
			$leadAnalytics->setLimit(static::getLimitComponentParams(RepeatLead::BOARD_KEY), Limit::isAnalyticsLimited(RepeatLead::BOARD_KEY));
			$leadAnalytics->setStepperEnabled(true);
			$leadAnalytics->setStepperIds(['crm' => ['Bitrix\Crm\Agent\History\LeadStatusSupposedHistory']]);
			$analyticPageList[] = $leadAnalytics;
		}
		...
		$contactDynamic->setLimit(static::getLimitComponentParams('contact_dynamic'), Limit::isAnalyticsLimited('contact_dynamic'));
