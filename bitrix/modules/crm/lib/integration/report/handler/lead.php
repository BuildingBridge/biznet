...
class Lead extends Base implements IReportSingleData, IReportMultipleData, IReportMultipleGroupedData
{
	...
	public function getUserInfo($userId)
	{
		...
		if(!$userId)
		{
			return ['name' => Loc::getMessage('CRM_REPORT_LEAD_USER_DEFAULT_NAME')];
		}
		...
		if(!$users[$userId])
		{
			...
			$userName =  !empty($userName) ? $userName : Loc::getMessage('CRM_REPORT_LEAD_USER_DEFAULT_NAME');
	...
	public function getTargetUrl($baseUri, $params = [])
	{
		...
		switch ($calculateValue)
		{
			case self::WHAT_WILL_CALCULATE_LOST_LEAD_COUNT:
				...
				$uri->addParams(['STATUS_SEMANTIC_ID' => 'F']);
				$result = $uri->getUri();
				break;
			case self::WHAT_WILL_CALCULATE_SUCCESS_LEAD_DATA_FOR_FUNNEL:
			case self::WHAT_WILL_CALCULATE_CONVERTED_LEAD_COUNT:
				$uri = new Uri($result);
				$uri->deleteParams(['STATUS_SEMANTIC_ID']);
				$uri->addParams(['STATUS_SEMANTIC_ID' => 'S']);
				$result = $uri->getUri();
				break;
		}
