...
class Deal extends Base implements IReportSingleData, IReportMultipleData, IReportMultipleGroupedData
{
	...
	private function addToQueryFilterCase(Query $query)
	{
		...
		foreach ($filterParameters as $key => $value)
		{
			if ($key === 'TIME_PERIOD')
			{
				...
				{
					$toDateValue = $this->getConvertedToServerTime($value['to']);
					$fromDateValue = $this->getConvertedToServerTime($value['from']);
	...
	public function getTargetUrl($baseUri, $params = [])
	{
		...
		switch ($calculateValue)
		{
			...
			case self::WHAT_WILL_CALCULATE_DEAL_LOSES_SUM:
				...
				$uri->addParams(['STAGE_SEMANTIC_ID' => 'F']);
				$result = $uri->getUri();
				break;
			case self::WHAT_WILL_CALCULATE_RETURN_DEAL_WON_SUM:
			case self::WHAT_WILL_CALCULATE_SUCCESS_DEAL_DATA_FOR_FUNNEL:
			...
			case self::WHAT_WILL_CALCULATE_DEAL_WON_COUNT:
				$uri = new Uri($result);
				$uri->deleteParams(['STAGE_SEMANTIC_ID']);
				$uri->addParams(['STAGE_SEMANTIC_ID' => 'S']);
				$result = $uri->getUri();
				break;
		}
	...
	public function getUserInfo($userId)
	{
		...
		if (!$userId)
		{
			return ['name' => Loc::getMessage('CRM_REPORT_DEAL_USER_DEFAULT_NAME')];
		}
		...
		if (!$users[$userId])
		{
			...
			$userName =  !empty($userName) ? $userName : Loc::getMessage('CRM_REPORT_DEAL_USER_DEFAULT_NAME');
