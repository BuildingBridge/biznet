...
class CAllCrmLead
{
	...
	public static function __AfterPrepareSql(/*CCrmEntityListBuilder*/ $sender, $arOrder, $arFilter, $arGroupBy, $arSelectFields)
	{
		...
		if (!empty($arFilter['ACTIVE_TIME_PERIOD_from']) && !empty($arFilter['ACTIVE_TIME_PERIOD_to']))
		{
			...
			$supposedHistoryCaseSql .= "LSHWS.LAST_UPDATE_DATE <= ".$DB->CharToDateFunction($arFilter['ACTIVE_TIME_PERIOD_to'], 'SHORT');
			$supposedHistoryCaseSql .=" AND LSHWS.CLOSE_DATE >= ".$DB->CharToDateFunction($arFilter['ACTIVE_TIME_PERIOD_from'], 'SHORT');

			if (!empty($arFilter['STATUS_SEMANTIC_ID']))
			{
				$statusSemanticIdsFromFilter = is_array($arFilter['STATUS_SEMANTIC_ID']) ? $arFilter['STATUS_SEMANTIC_ID'] : array($arFilter['STATUS_SEMANTIC_ID']);
				$supposedHistoryCaseSql  .= "AND  LSHWS.STATUS_SEMANTIC_ID  IN (" . implode(',', array_map(function($el) {return "'" . $el . "'";}, $statusSemanticIdsFromFilter)) . ")";
			}
