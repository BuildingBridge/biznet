...
class CAllCrmDeal
{
	...
	public static function __AfterPrepareSql(/*CCrmEntityListBuilder*/ $sender, $arOrder, $arFilter, $arGroupBy, $arSelectFields)
	{
		...
		if (!empty($arFilter['ACTIVE_TIME_PERIOD_from']) && !empty($arFilter['ACTIVE_TIME_PERIOD_to']))
		{
			...
			$supposedHistoryCaseSql .= "DSHWS.LAST_UPDATE_DATE <= ".$DB->CharToDateFunction($arFilter['ACTIVE_TIME_PERIOD_to'], 'SHORT');
			$supposedHistoryCaseSql .=" AND DSHWS.CLOSE_DATE >= ".$DB->CharToDateFunction($arFilter['ACTIVE_TIME_PERIOD_from'], 'SHORT');

			if (!empty($arFilter['STAGE_SEMANTIC_ID']))
			{
				$stageSemanticIdsFromFilter = is_array($arFilter['STAGE_SEMANTIC_ID']) ? $arFilter['STAGE_SEMANTIC_ID'] : array($arFilter['STAGE_SEMANTIC_ID']);
				$supposedHistoryCaseSql  .= "AND  DSHWS.STAGE_SEMANTIC_ID  IN (" . implode(',', array_map(function($el) {return "'" . $el . "'";}, $stageSemanticIdsFromFilter)) . ")";
			}
