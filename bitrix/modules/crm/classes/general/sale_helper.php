...
class CCrmSaleHelper
{
	...
	private static function setCurrentUserToGroup($groupId)
	{
		...
		if ($groupId)
		{
			...
			{
				$groups[] = $groupId;
				CUser::SetUserGroup($userId, $groups, false);
	...
	public static function addUserToShopGroup($newUserIds = [], $currentUserIds = [])
	{
		if (IsModuleInstalled("bitrix24"))
		{
			...
			$intersectIds = array_intersect($newUserIds, $currentUserIds);
			$userIdsToDelete = array_diff($currentUserIds, $intersectIds);
			self::deleteUserFromShopGroupByUserIds($userIdsToDelete);
		}
	}
	...
	private static function deleteUserFromShopGroupByUserIds($currentUserIds = [])
	{
		...
		if ($connection->isTableExists("b_user_group"))
		{
			...
			{
				$groupId = self::getShopGroupIdByUserId($userId);
				if (!is_array($dataToQuery[$groupId]))
				{
					$dataToQuery[$groupId] = [];
				}

				$dataToQuery[$groupId][] = $userId;
			}

			if ($dataToQuery)
			{
				foreach ($dataToQuery as $groupId => $listUserId)
				{
					$strSql = "DELETE FROM b_user_group WHERE GROUP_ID = $groupId and USER_ID IN (" .
						implode(',', $listUserId) . ")";
					$connection->queryExecute($strSql);
