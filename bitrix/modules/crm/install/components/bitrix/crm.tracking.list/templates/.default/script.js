;(function ()
{
	...
	Grid.prototype.init = function (params)
	{
		...
		BX.addCustomEvent(
			...
			function (tile) {
				...
				{
					this.showItemPopup(tile.node, tile.data.items);
				}
				else
				{
					BX.SidePanel.Instance.open(tile.data.url, {width: 735, cacheable: false});
	...
	Grid.prototype.showItemPopup = function (node, items)
	{
		if (this.itemPopup)
		{
			this.itemPopup.toggle();
		}
		...
		popupItems = popupItems.map(function (item) {
			...
			if (BX.type.isBoolean(item.active))
			{
				...
				item.text = '<span class="crm-tracking-list-popup-item ' + className + '"></span>' + item.text;
			}
		...
		this.itemPopup = BX.PopupMenu.create(
			"crm-tracking-list-channel-items",
			node, //bindElement
			popupItems,
			{
				maxHeight: 250,
