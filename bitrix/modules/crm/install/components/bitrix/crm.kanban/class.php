...
class CrmKanbanComponent extends \CBitrixComponent
{
	...
	/**
	 *
	 * @param int $entityId Entity id, or ids.
	 * @return array
	 */
		...
		if (class_exists($provider))
		{
				...
				foreach ($addSelect as $code)
				{
					if (array_key_exists($code, $row) && array_key_exists($code, $addFields))
					{
						...
						)
						{
							...
							{
								$dateFormat = $this->dateFormats[date('Y') == date('Y', $row['DATE_UNIX']) ? 'short' : 'full'][$this->formatLang];
								$row[$code] = \FormatDate($dateFormat, \MakeTimeStamp($row[$code]), $timeFull);
							}
			...
			if ($users)
			{
				...
				while ($row = $res->fetch())
				{
					...
					// tmp
					$users[$row['ID']] = \htmlspecialcharsbx($users[$row['ID']]['title']);
					$users[$row['ID']] = '<a href="' . $link . '">' . $users[$row['ID']] . '</a>';
				}
				unset($res, $row);
			}
			// get file's infos
			...
			// tracking analytic
			$traces = [];
			if (in_array('TRACKING_SOURCE_ID', $select))
			{
		...
		if (is_array($config) && !empty($config))
		{
			foreach ($config as $configSection)
			{
				...
				)
				{
					...
					if (in_array($configSection['name'], ['main', 'additional']))
					{
