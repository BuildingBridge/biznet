...
class CMobileHelper
{
	...
	public static function createLink($tag)
	{
		...
		else if (substr($tag, 0, 7) == 'RATING|')
		{
			...
			if ($params[1] == 'TASK')
			{
				$result = SITE_DIR.'mobile/tasks/snmrouter/?routePage=view&USER_ID='.intval($USER->getId()).'&TASK_ID='.intval($params[2]);

				// TODO: uncomment after fix all bug in TASKS in branch
				//$taskId = intval($params[2]);
                //return 'BXMobileApp.Events.postToComponent("taskbackground::task::action", [{taskId: '.$taskId.', selectedTab: "taskTab"}], "background");';
			}
		...
		)
		{
			...
			$tab = 'taskTab';
			if(substr($tag, 0, 14) == 'TASKS|COMMENT|')
            {
                $tab = 'commentTab';
            }

			// TODO: uncomment after fix all bug in TASKS in branch
            //return 'BXMobileApp.Events.postToComponent("taskbackground::task::action", [{taskId: '.$taskId.', selectedTab: "'.$tab.'"}], "background");';
