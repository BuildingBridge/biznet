	...
	)
	{
		...
		if ($arParams["IS_LIST"])
		{
			...
			if ($taskId)
			{
				...
				if(0 && \Bitrix\MobileApp\Mobile::getApiVersion() >= 29)
				{
					\CModule::includeModule('tasks');
					$task = \CTaskItem::getInstance($taskId, \Bitrix\Tasks\Util\User::getId());
					$strOnClick = " onclick=\"openTaskComponentByTaskId(event, $taskId, {publicUrl: '" . \Bitrix\MobileApp\Janative\Manager::getComponentPath("tasks.view") . "', title:'" . htmlspecialcharsbx($task['TITLE']) . "'})\"";
				}
				else
				{
		...
		?><div class="<?=($item_class)?>" id="lenta_item_<?=$arEvent["EVENT"]["ID"]?>"><?
			...
			><?
				...
				)
				{
					...
					if(0 && $taskId && \Bitrix\MobileApp\Mobile::getApiVersion() >= 29)
					{
						$strOnClickCommentsTop = " onclick=\"openTaskComponentByTaskId(event, $taskId, {selectedTab: 'commentTab', publicUrl: '".\Bitrix\MobileApp\Janative\Manager::getComponentPath("tasks.view")."', title: '".htmlspecialcharsbx($task['TITLE'])."' })\"";
					}
					else
					{
