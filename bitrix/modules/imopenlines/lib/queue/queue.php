...
abstract class Queue
{
	...
	public function isOperatorAvailable($userId, $currentOperator = 0)
	{
		...
		if(ImOpenLines\Queue::isOperatorActive($userId, $this->config['TIMEMAN']))
		{
	...
	public function createSession($operatorId = 0, $crmManager = null, $isGroupByChat = false)
	{
		...
		if(empty($operatorId))
		{
			...
			else
			{
				...
				{
					$result['UNDISTRIBUTED'] = true;
					$result['DATE_QUEUE'] = (new DateTime())->add(ImOpenLines\Queue::UNDISTRIBUTED_QUEUE_TIME . ' SECONDS');
				}
	...
	public function isActiveCrmUser($idUser)
	{
		...
		if ($result != false && !Im\User::getInstance($idUser)->isActive())
		{
			$result = false;
		}

		if ($result != false && Im\User::getInstance($idUser)->isAbsent())
		{
			$result = false;
		}

		if ($result != false && $this->config['TIMEMAN'] == "Y" && !ImOpenLines\Queue::getActiveStatusByTimeman($idUser))
		{
			$result = false;
		}

		Log::write(['idUser' => $idUser, 'Result' => $result], 'IS ACTIVE CRM USER');
