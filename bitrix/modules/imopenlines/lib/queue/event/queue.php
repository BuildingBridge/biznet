...
abstract class Queue
{
	...
	public function getCountFreeSlots()
	{
		...
		if($this->configLine['CHECK_ONLINE'] == 'Y')
		{
		...
		{
			if(ImOpenLines\Queue::isOperatorActive($queueUser['USER_ID'], $this->configLine['TIMEMAN']))
			{
				if($this->configLine['CHECK_ONLINE'] != 'Y' ||
	...
	{
		if(empty($this->configLine['MIN_DAYS_TO_CHECK_DIALOGS']))
		{
			$minDaysAbsence = 1;
		}
		else
		{
			$minDaysAbsence = $this->configLine['MIN_DAYS_TO_CHECK_DIALOGS'];
		}

		//not accepted
		if($minDaysAbsence > $durationAbsenceDay)
		{
		...
		{
			$this->returnSessionsUsersToQueue($userId, $reasonReturn);
		}
	}
