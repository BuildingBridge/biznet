...
class Chat
{
			...
			if(DbNameLock::set($answerTag))
			{
				...
				if($result->isSuccess())
				{
					...
					foreach ($relations as $relation)
					{
						...
						if ($config['WELCOME_BOT_LEFT'] == Config::BOT_LEFT_CLOSE && ImUser::getInstance($relation['USER_ID'])->isBot())
							continue;
	 ...
	 * @throws Main\SystemException
	 */
	public static function getLastChatIdByCrmEntity($crmEntityType, $crmEntityId)
	{
		$result = 0;

		if (Main\Loader::includeModule('im') && Main\Loader::includeModule('crm'))
		{
			$filter = [
				'BINDINGS' =>
					[
						0 =>
							[
								'OWNER_TYPE_ID' => Crm\Common::getCrmEntityIdByTypeCode($crmEntityType),
