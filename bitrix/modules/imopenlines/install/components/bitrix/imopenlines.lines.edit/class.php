...
class ImOpenLinesComponentLinesEdit extends CBitrixComponent implements Controllerable
{
	...
	private function updateLine()
	{
		...
		if(empty($post['CONFIG']['MIN_DAYS_TO_CHECK_DIALOGS']) && !is_numeric($post['CONFIG']['MIN_DAYS_TO_CHECK_DIALOGS']) && $post['CONFIG']['MIN_DAYS_TO_CHECK_DIALOGS'] < 1)
		{
			$post['CONFIG']['MIN_DAYS_TO_CHECK_DIALOGS'] = 1;
		}
		else
		{
			$post['CONFIG']['MIN_DAYS_TO_CHECK_DIALOGS'] = round($post['CONFIG']['MIN_DAYS_TO_CHECK_DIALOGS']);
		}
	...
	private function showConfig()
	{
		...
		{
			$this->arResult['VISIBLE']['CHECK_OFFLINE_BLOCK'] = true;
			$this->arResult['VISIBLE']['CHECK_ONLINE_BLOCK'] = false;
		...
		{
			if($this->arResult['CONFIG']['CHECK_ONLINE'] != 'Y')
			{
				$this->arResult['VISIBLE']['CHECK_OFFLINE_BLOCK'] = false;
			}
			else
			{
				$this->arResult['VISIBLE']['CHECK_OFFLINE_BLOCK'] = true;
			}
			$this->arResult['VISIBLE']['CHECK_ONLINE_BLOCK'] = true;
