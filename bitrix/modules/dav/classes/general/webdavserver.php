	...
	extends CDavWebDav
{
	...
	protected function PROPFIND(&$arResources, $method = 'PROPFIND')
	{
		...
		if ($matches = $this->isStorage($requestPath))
		{
			...
			$quotaSize = $this->countQuotaSizeStorage($storage);
		...
		$arResources[] = $this->getResourceByObject($requestPath, $object, $quotaSize);
...
//			return '409 Conflict';
	 ...
	 * @return CDavResource
	 */
	protected function getResourceByObject($path, BaseObject $object, $quotaSize = null)
	{
		...
		if ($quotaSize)
		{
			$resource->AddProperty("quota-available-bytes", $quotaSize['available_size']);
			$resource->AddProperty("quota-used-bytes", $quotaSize['used_size']);
		}
	...
	private function fillSystemStorage(&$arResources, $requestPath)
	{
		if ($requestPath === '/docs/')
		{
			$quotaSize = $this->countQuotaSizeDisk();

			$arResources[] = $this->getResourceObjectRoot("", $quotaSize);
			if ($this->request->GetDepth())
			{
				$arResources[] = $this->getResourceObjectRoot(Loc::getMessage('DAV_USER_STORAGES'), $quotaSize);
				$arResources[] = $this->getResourceObjectRoot(Loc::getMessage('DAV_GROUP_STORAGES'), $quotaSize);
	...
	protected function getResourceObjectRoot($path, $quotaSize = null)
	{
	...
	private function fillResourceRootStorages ($entityType, &$arResources, $requestPath)
	{
		...
		foreach ($storages as $objectId => $item)
		{
			...
			$arResources[] = $this->getResourceByObject($item['URL'], $object, $quotaSize);
		}
